...
def execute():
    ...
    city = cityBox.get()
    resultBox.insert(END, name + " " + surname + " " + city)
    refresh()
...
cityLabel = Label(window, text = "City:", font="Calibri 14")
...
cityBox = Combobox(window, values = city, width = 15, font="Calibri 14")
cityBox['state'] = 'readonly'
cityBox.current(0)
cityBox.grid(row=2, padx=20, pady=20, column = 1)
